...
const DefaultPromptTmpl = `你需要解决一个问题。为此，你需要将问题分解为多个步骤。对于每个步骤，首先使用 <thought> 思考要做什么，然后使用可用工具之一决定一个 <action>。接着，你将根据你的行动从环境/工具中收到一个 <observation>。持续这个思考和行动的过程，直到你有足够的信息来提供 <final_answer>。
如果不需要使用工具，则跳过<thought>，直接给出答案即可。
...
func FormatPrompt(toolsList, os, currentDir, fileList string) string {
	return fmt.Sprintf(DefaultPromptTmpl, toolsList, "", os, currentDir, fileList)
}

func FormatPromptWithSkills(toolsList, skillsList, os, currentDir, fileList string) string {
	skillsList = fmt.Sprintf("本次任务使用的技能清单：\n%s", skillsList)
	return fmt.Sprintf(DefaultPromptTmpl, toolsList, skillsList, os, currentDir, fileList)
}
